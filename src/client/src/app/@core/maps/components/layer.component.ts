...
import {Cluster, ImageWMS, OSM, Source, TileWMS, WMTS, XYZ} from 'ol/source';
...
export class LayerComponent implements OnInit, OnDestroy, OnChanges {
   ...
   */
  @Output() layerReady = new AsyncSubject(); // AsyncSubject will only store the last value, and only publish it when the sequence is completed
  ...
  ngOnInit() {
    ...
    if (this.layer && map) {
      this.layer.setZIndex(this.zIndex || map.getLayers().getLength());
  ...
  getSource(): Source {
    let olSource: any = this.layer.getSource();
